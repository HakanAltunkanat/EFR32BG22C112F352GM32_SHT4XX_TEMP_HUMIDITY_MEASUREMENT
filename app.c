...
extern I2CSPM_Init_TypeDef init_inst;

float temperature, humidity;
uint8_t deviceConnected=0;
uint8_t bt_TxBuffer[4];
int leftT,rightT, leftRH, rightRH;

void split_float(float x, int *left2, int *right2) {
    double intpart, fracpart;

    fracpart = modf((double)x, &intpart);


    int abs_int = (int)fabs(intpart);
    *left2 = abs_int % 100;



    *right2 = (int)((fabs(fracpart) * 100.0) + 0.0001) % 100;
}
...
SL_WEAK void app_init(void)
{
  ...
  I2CSPM_DeInit(&init_inst); //deinit i2c
  GPIO_PinModeSet(gpioPortA, 0, gpioModeDisabled, 0); //turn off the sensor supply
  /////////////////////////////////////////////////////////////////////////////
}
...
SL_WEAK void app_process_action(void)
{
  if(deviceConnected)
    {
      sht40_Measure(&temperature, &humidity); //get the measurement results
      split_float(temperature, &leftT, &rightT); //divide result into two part (left of decimal point and right of decimal point)
      split_float(humidity, &leftRH, &rightRH);
      bt_TxBuffer[0]=leftT;
      bt_TxBuffer[1]=rightT;
      bt_TxBuffer[2]=leftRH;
      bt_TxBuffer[3]=rightRH;
      sl_bt_gatt_server_notify_all(gattdb_tx, 4, bt_TxBuffer); //send the results
      delayMs(500);
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_opened_id:
          GPIO_PinModeSet(gpioPortA, 0, gpioModePushPull, 1); //turn on the sensor supply
          delayMs(100); //wait for warmup
          deviceConnected=1; //set the device connection flag
          I2CSPM_Init(&init_inst); //init i2c
         break;
       case sl_bt_evt_connection_closed_id:
        deviceConnected=0;
        GPIO_PinModeSet(gpioPortA, 0, gpioModeDisabled, 0);
        I2CSPM_DeInit(&init_inst);
        //start advertising
...
static void bcn_setup_adv_beaconing(void)
{
  ...
  sc = sl_bt_advertiser_set_timing(
    advertising_set_handle,
    3200,     // min. adv. interval (milliseconds * 1.6) //advertise 2 seconds period
